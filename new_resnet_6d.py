...
__all__ = ['ResNet', 'resnet18', 'resnet34', 'resnet50', 'resnet101',
           'resnet152']
from config import useHead, without_closs, use_transform
...
class ResNet_old(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(regress_e.reshape(regress_e.size(0), -1).size())
...
class ResNet_audio(nn.Module):
    ...
    def forward(self, x):
        ...
        if without_closs:
            x_reg = [(self.rfc_e(re_e.reshape(re_e.size(0), -1))),
                     (self.rfc_n(re_n.reshape(re_n.size(0), -1))),
                     (self.rfc_a(re_a.reshape(re_a.size(0), -1))),
                     (self.rfc_c(re_c.reshape(re_c.size(0), -1))),
                     (self.rfc_o(re_o.reshape(re_o.size(0), -1))),
                     (self.rfc_i(re_i.reshape(re_i.size(0), -1))), ]
...
def resnet34_old(pretrained=False, **kwargs):
    """Constructs a ResNet-34 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
